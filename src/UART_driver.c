...
void UART_init(uint32_t ui32Base)
{
    ...
    volatile uint32_t *RCGCUART_pointer = (uint32_t*)RCGCUART;
    volatile uint32_t *RCGCGPIO_pointer = (uint32_t*)RCGCGPIO;
    ...
    volatile uint32_t *UARTCTL_pointer = (uint32_t*)(ui32Base + UARTCTL);
    volatile uint32_t *UARTIBRD_pointer = (uint32_t*)(ui32Base + UARTIBRD);
    volatile uint32_t *UARTFBRD_pointer = (uint32_t*)(ui32Base + UARTFBRD);
    volatile uint32_t *UARTLCRH_pointer = (uint32_t*)(ui32Base + UARTLCRH);
    volatile uint32_t *UARTCC_pointer = (uint32_t*)(ui32Base + UARTCC);
    //-----------------------------------------------------------------------------
    ...
    switch(ui32Base)
    {
        ...
        case UART_base_0 :
            ...
            // UART Receive (Rx) and transmit (Tx) for UART module 0 is on pin 0 and 1 (bits 7 to 0)
            UART_pin_bits = 0xFF;
            break;
        // UART base 1
        case UART_base_1 :
            ...
            // UART Receive (Rx) and transmit (Tx) for UART module 1 is on pin 0 and 1 (bits 7 to 0)
            UART_pin_bits = 0xFF;
            break;
        // UART base 2
        case UART_base_2 :
            ...
            // Set which GPIO Port base is to be used
            temp_port_base = GPIO_Port_A_base;
            // UART Receive (Rx) and transmit (Tx) for UART module 0 is on pin 6 and 7 (bits 31 to 24)
            UART_pin_bits = 0xFF000000;
            break;
        // UART base 3
        case UART_base_3 :
            // UART module 3
            ...
            // Set which GPIO Port base is to be used
            temp_port_base = GPIO_Port_A_base;
            // UART Receive (Rx) and transmit (Tx) for UART module 0 is on pin 4 and 5 (bits 23 to 16)
            UART_pin_bits = 0xFF0000;
            break;
        // UART base 4
        case UART_base_4 :
            // UART module 4
            ...
            // Set which GPIO Port base is to be used
            temp_port_base = GPIO_Port_A_base;
            // UART Receive (Rx) and transmit (Tx) for UART module 0 is on pin 2 and 3 (bits 15 to 8)
            UART_pin_bits = 0xFF00;
            break;
        // UART base 5
        case UART_base_5 :
            // UART module 5
    ...
    GPIOAFSEL_pointer = (uint32_t*) (temp_port_base + GPIOAFSEL);
    GPIODR2R_pointer = (uint32_t*) (temp_port_base + GPIODR2R);
    GPIOSLR_pointer = (uint32_t*) (temp_port_base + GPIOSLR);
    GPIOPCTL_pointer = (uint32_t*) (temp_port_base + GPIOPCTL);
    GPIODEN_pointer = (uint32_t*) (temp_port_base + GPIODEN);
    //-----------------------------------------------------------------------------
    ...
    *UARTLCRH_pointer |= (1 << 5);
    *UARTLCRH_pointer |= (1 << 6);
    ...
    *UARTCC_pointer &= ~(0xF);
    ...
    // The transmit section of the UART is enabled (bit 8). The receive section of the UART is enabled (bit 9). But these must be activated at the same time as UARTEN. Enable UARTEN bit (bit 0).
    // 1100000001 in binary is 0x301 in hex
    //*UARTCTL_pointer |= (0x301);
    *UARTCTL_pointer |= ((1 << 8) | (1 << 9) | (1 << 0));
...
{
    //-----------------------------------------------------------------------------
    volatile uint32_t *UARTDR_pointer = (uint32_t*) (g_UART_base_used + UARTDR);
    volatile uint32_t *UARTRSR_ECR_pointer = (uint32_t*) (g_UART_base_used + UARTRSR_ECR);
    volatile uint32_t *UARTFR_pointer = (uint32_t*) (g_UART_base_used + UARTFR);
    ...
    *UARTRSR_ECR_pointer = 0x00000000;
    ...
    character_received = *UARTDR_pointer;
...
void UART_putChar(char c)
{
    ...
    if (g_uart_init == 1)
    {
        ...
        *UARTRSR_ECR_pointer = 0x00000000;
        // Clear the data register before writing to it
        *UARTDR_pointer = 0x00000000;
...
void UART_reset()
{
    ...
    *RCGCGPIO_pointer = 0x7FFF;
    ...
    for (i = 0; i < 15; i++)
    {
        GPIODR2R_pointer = (uint32_t*) (port_arr[i] + GPIODR2R);
        *GPIODR2R_pointer = 0x000000FF;
        GPIOSLR_pointer = (uint32_t*) (port_arr[i] + GPIOSLR);
...
void UART_getString(char *buffer)
{
    ...
    // Continue to read as long as there is data that can be received. Bit 6 is 0 when the receiver can receive data.
    while (((*UARTFR_pointer) & (1 << 6)) == 0)
    {
